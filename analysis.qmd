...
#county_sf <- st_read("dataset/tl_2023_us_county.shp")
#print(county_sf)
...
# Filtering for New York State counties only
#ny_county_sf <- county_sf %>%
#  filter(STATEFP == '36')

# Simplifying the geometries for New York State
#simplified_ny_county_sf <- st_simplify(ny_county_sf, preserveTopology = TRUE, dTolerance = 0.01)

# Merging the datasets, making sure to merge the simplified New York spatial data
#merged_data <- simplified_ny_county_sf %>%
#  left_join(clean_data, by = c("NAME" = "County"))

#class(merged_data)
#str(merged_data)

# Ploting the map using a simpler theme for speed
#map_plot <- ggplot(data = merged_data) +
#  geom_sf(aes(fill = Race_Ethnicity)) +  # Replace 'Race_Ethnicity' as needed
#  scale_fill_viridis_d(option = "plasma", begin = 0, end = 1, direction = 1, guide = guide_legend(title = "Race/Ethnicity")) +
#  theme_void()  # Using theme_void for minimal styling to improve rendering speed
