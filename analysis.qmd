...
Model 2: Linear Regression between Household weight, Education, Income group, Race/Ethnicity, 

```{r}
cleaned_data$Education <- as.factor(cleaned_data$Education.Level)
cleaned_data$Race_Ethnicity <- as.factor(cleaned_data$Race...Ethnicity)
cleaned_data$Income_group <- as.factor(cleaned_data$Income.Groups)
cleaned_data$Race_Ethnicity <- as.factor(cleaned_data$Economic.Development.Region)
cleaned_data$Race_Ethnicity <- as.factor(cleaned_data$Owner.Renter.Status)
cleaned_data$Race_Ethnicity <- as.factor(cleaned_data$Household.Type)

# Linear model
model_linear <- lm(Household.Weight ~ Education.Level + Race...Ethnicity + Income.Groups + Economic.Development.Region + Owner.Renter.Status + Household.Type, data = cleaned_data)
